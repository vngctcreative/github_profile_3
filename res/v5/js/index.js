...
class MusicPlayer {
    ...
    MusicPlay(event) {
        ...
        const u = selectedElement.attr("url");

        if (this.u === u && this.a && !this.a.paused) { // If the same song is clicked and is currently playing
            this.a.pause();
            selectedElement.find(".title").text(this.tt); // Restore original title
            WsToast('info', 'Tạm dừng: ' + this.tt);
            return;
        } else if (this.u === u && this.a && this.a.paused) { // If the same song is clicked and is paused
            this.a.play();
            selectedElement.find(".title").text("Đang phát: " + this.tt);
            WsToast('success', 'Tiếp tục phát: ' + this.tt);
            return;
        }
        ...
        Wstoast.closeAll();
        if (u) {
            let t = selectedElement.find(".title");
            let o = t.text();
...
$(document).ready(function() {
    // Xử lý hiện/ẩn giao diện Profile và Đếm ngày yêu
    var icon = $('.mode-loveday').find('i');
    if (GetCookie("IconState") == "home") {
        icon.removeClass('ri-heart-2-line').addClass('ri-home-line');
    } else {
    ...
    $('.mode-loveday').click(function() {
        ...
        if (icon.hasClass('ri-heart-2-line')) {
            ...
            icon.removeClass('ri-heart-2-line').addClass('ri-home-line');
            document.cookie = "LoveDaysVisible=true; expires=Fri, 31 Dec 9999 23:59:59 GMT";
            $('.language-skills').hide(); // Ẩn phần "Language Skill"
        } else {
            document.cookie = "IconState=heart; expires=Fri, 31 Dec 9999 23:59:59 GMT";
            Wstoast.closeAll();
            WsToast('info', 'Đã chuyển về chế độ profile');
            icon.removeClass('ri-home-line').addClass('ri-heart-2-line');
            document.cookie = "LoveDaysVisible=false; expires=Fri, 31 Dec 9999 23:59:59 GMT";
            $('.language-skills').show(); // Hiện phần "Language Skill"
        }
        $('.td-content').slideToggle();
        $('#td-lovedays').slideToggle();
    });
    
    // Kiểm tra trạng thái và ẩn/hiện phần "Language Skill" khi tải trang
    if (GetCookie("LoveDaysVisible") == "true") {
        $('.language-skills').hide();
    } else {
